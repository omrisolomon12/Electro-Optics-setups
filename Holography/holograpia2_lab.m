...
theta_x = 10000*lambda;
theta_y = 100;

% Object- make a square object

object=zeros(N,M); 
object(500:524,500:524)=1; 

% H_tot=double(imread('v_lines.tif'));
% [y,x]=size(t);
% normal_of_holog(1:y,1:x)=(t);
% 
% object = read(t);
...
Ref_beam=1*exp(((2.*pi.*1i)/(lambda)).*(d+theta_x.*X_ccd+theta_y.*Y_ccd));

%Hologram Intensety 
%H_tot = (abs(U_obj+Ref_beam)).^2;
%H_tot = (abs(normal_of_holog+Ref_beam)).^2;
H_tot=double(imread('1.tif'));
figure;
imagesc(H_tot); 
